...
function TodoCard ({ todo, index, handleCompleteTodo, handleDeleteTodo }) {
  ...
  return (
    <div className="bg-gray-200 mx-2 my-4 rounded-md p-2 font-[500] max-w-[650px]">
      ...
      <div>
            <button  onClick={()=> handleCompleteTodo(index)} disabled={todo.complete} className={`${btnsStyle} bg-sky-500 disabled:bg-sky-300 disabled:cursor-not-allowed`}>
              <h6>Done</h6>
            </button>
            <button  onClick={()=> handleDeleteTodo(index)} className={`${btnsStyle} bg-purple-300`}>
              <h6>Delete</h6>
            </button>
      </div>
